...
public class UserServiceTest {
	...
	//@Test
	 public void testUpdateUser() throws Exception{
		...
		User user = userService.getUser("fff3301");
		System.out.println(user+"여기는 user");
		Assert.assertNotNull(user);
		
		Assert.assertEquals("박지훈", user.getUserName());
		Assert.assertEquals("777-7777-7777", user.getPhone());
		Assert.assertEquals("change", user.getAddr());
		Assert.assertEquals("gkstj8300@naver.com", user.getEmail());

		user.setUserName("박한서");
		user.setPhone("888-8888-8888");
		user.setAddr("change2");
		user.setEmail("gkstj8301@naver.com");
		
		userService.updateUser(user);
		
		user = userService.getUser("fff3301");
		Assert.assertNotNull(user);
		...
		//==> API 확인
		Assert.assertEquals("박한서", user.getUserName());
		Assert.assertEquals("888-8888-8888", user.getPhone());
		Assert.assertEquals("change2", user.getAddr());
		Assert.assertEquals("gkstj8301@naver.com", user.getEmail());
	 }
...
//		userService.addUser(user);
	 ...
	 public void testGetUserListAll() throws Exception{
	 	...
	 	System.out.println("여기는 list"+list);
		System.out.println("총 검색 갯수 :: "+map.get("totalCount"));	
		System.out.println(list.size()+" list.size()입니다");
